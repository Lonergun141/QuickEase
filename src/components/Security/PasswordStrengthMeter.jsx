...
const PasswordStrengthMeter = ({ password }) => {
	...
	const [hidePasswordMeter, setHidePasswordMeter] = useState(false);
	...
	useEffect(() => {
		...
		setHidePasswordMeter(false);

		if (strength === 'strong' && password.length >= 12) {
			const timer = setTimeout(() => {
				setHidePasswordMeter(true);
			}, 5000);
					...
					<div className="p-4 space-y-5">
						...
						<div className="flex items-center justify-between">
							<div className="flex items-center space-x-3">
								<div
									className={`p-2 rounded-lg bg-${strengthConfig.color}-50 dark:bg-${strengthConfig.color}-500/10`}>
								...
								<div
									...
									<div
										className={`flex-shrink-0 ${
											req.met ? `text-${strengthConfig.color}-500` : ''
										}`}>
										{req.met ? (
